...
func (s *workoutService) SendTodaysWorkoutEmail() error {
	...
	settings, err := s.repo.GetWorkoutEmailSettings(context.Background())
	...
	if emailData.RestDay {
		...
		subject = fmt.Sprintf("Today's workout: %s", schedule.WorkoutName)
	}
	...
	return s.emailService.SendWorkoutEmail(
		settings.Email,
		subject,
		htmlBody,
	)
